    ...
    override func spec() {
        ...
        describe("when performing operation") {
            ...
            it("completes with resulting errors") {
                ...
                expect(errors[0]).toEventually(beNil())
                expect(errors[1]).toEventually(be(error))
                expect(errors[2]).toEventually(beNil())
                expect(errors[3]).toEventually(be(error))
                expect(errors[4]).toEventually(beNil())
            }
        }
    }
}
